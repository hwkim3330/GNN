...
# 0. 설정
# ==============================================================================
# CPU 코어 수에 맞게 설정 (e.g., 8코어 -> 8)
NUM_THREADS = 8
os.environ['OMP_NUM_THREADS'] = str(NUM_THREADS)
os.environ['MKL_NUM_THREADS'] = str(NUM_THREADS)
torch.set_num_threads(NUM_THREADS)

IMITATION_EPOCHS = 250
IMITATION_DATASET_SIZE = 200
IMITATION_LR = 1e-4
BENCHMARK_SCENARIOS = 100
DEVICE = torch.device("cpu")

MODEL_PATH = "gnn_path_scorer_final_v6.pth"
RESULT_PLOT_PATH = "benchmark_results_final_v6.png"
K_SHORTEST_PATHS = 5
...
class RealisticProfileGenerator:
    def _generate_fat_tree(self, k):
        ...
        num_pods, num_core_switches = k, (k // 2) ** 2
        num_agg_switches, num_edge_switches = k * (k // 2), k * (k // 2)
        G = nx.Graph()
        core_switches, agg_switches, edge_switches = range(num_core_switches), range(num_core_switches, num_core_switches + num_agg_switches), range(num_core_switches + num_agg_switches, num_core_switches + num_agg_switches + num_edge_switches)
        for i in range(num_core_switches):
            for j in range(num_pods):
                G.add_edge(core_switches[i], agg_switches[j * (k // 2) + (i // (k // 2))], bandwidth=CORE_AGG_BANDWIDTH_BPS)
        for j in range(num_pods):
            for i in range(k // 2):
                for l in range(k // 2):
                    G.add_edge(agg_switches[j * (k // 2) + i], edge_switches[j * (k // 2) + l], bandwidth=AGG_EDGE_BANDWIDTH_BPS)
    ...
    def generate(self):
        ...
        num_servers = max(1, len(edge_switches) // 20)
...
class GNN_Path_Scorer_Solver(BaseSolver):
    def __init__(self, model_path):
        super().__init__("GNN-Scorer")
        self.model = PathScorerGNN(node_feature_dim=4)
    ...
    def solve(self, env, profile):
        ...
        for flow_id, props in sorted(all_flow_defs.items(), key=lambda item: item[1]['period_ms']):
            ...
            with torch.no_grad(): scores = self.model(net_batch, path_node_indices_list)
            p_path = candidate_paths[torch.argmax(scores).item()]
...
def run_full_procedure_and_benchmark():
    ...
    model = PathScorerGNN(node_feature_dim=4)
    ...
    print("\n===== 최종 랜덤 벤치마크 실행 =====")
    solvers = [GNN_Path_Scorer_Solver(MODEL_PATH), Greedy_Solver(), ILP_Solver(time_limit_sec=ILP_TIME_LIMIT_SEC)]
    results = []
