...
# --- 2단계: PPO 강화학습 ---
RL_EPISODES = 20000; RL_LR = 3e-5; GAMMA = 0.99; EPS_CLIP = 0.2
UPDATE_TIMESTEP = 2000 # PPO 업데이트 주기 (스텝 수 기준)
PPO_UPDATES = 10       # 한 번의 업데이트에 수행할 에폭 수
# --- 모델 및 벤치마크 ---
...
class ActorCriticGNN(nn.Module):
    def __init__(self, input_dim, hidden_dim=64):
        ...
        self.conv1_actor = GCNConv(input_dim, hidden_dim); self.conv2_actor = GCNConv(hidden_dim, hidden_dim); self.actor_head = nn.Linear(hidden_dim, 1)
        self.conv1_critic = GCNConv(input_dim, hidden_dim); self.conv2_critic = GCNConv(hidden_dim, hidden_dim); self.critic_head = nn.Linear(hidden_dim, 1)
    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        x_actor = F.relu(self.conv1_actor(x, edge_index)); x_actor = F.relu(self.conv2_actor(x_actor, edge_index)); action_logits = self.actor_head(x_actor).squeeze(-1)
        x_critic = F.relu(self.conv1_critic(x, edge_index)); x_critic = F.relu(self.conv2_critic(x_critic, edge_index)); state_value = self.critic_head(x_critic)
...
class PPOAgent:
    ...
    def select_action(self, state, memory, current_path):
        with torch.no_grad():
            ...
            action = dist.sample()
        if memory is not None: memory.states.append(state); memory.actions.append(action); memory.logprobs.append(dist.log_prob(action))
        return action.item()
...
def run_ppo_training(agent, profile_generator, episodes):
    ...
    for ep in pbar:
        ...
        if ep % 20 == 0: pbar.set_postfix(avg_reward=f"{running_reward/20:.4f}"); running_reward = 0
