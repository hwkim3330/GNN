...
static long get_link_speed_bps(const char *ifname) {
    char cmd[128];
...
// Normalize CBS\ void normalize_cbs(const char *ifname) {
    ...
    double hi_a = ceil(idle_a * fa / link_k);
    double lo_a = ceil(send_a * fa / link_k);
    CbsCredit cred_a = {
    ...
    double hi_b = ceil(idle_b * ((double)fb/(link_k-ia) + idle_a/link_k));
    double lo_b = ceil(send_b * fb / link_k);
    CbsCredit cred_b = {
...
void normalize_tas() {
    ...
    int sched_cnt = sizeof(tas_sched)/sizeof(tas_sched[0]);
    ...
    printf("[TAS] num_tc=%d\n map: ", next);
    for (int i = 0; i < 16; i++) printf("%d->%d ", i, tc_map[i]);
    printf("\nSched:\n");
    for (int i = 0; i < sched_cnt; i++) {
        TasSchedule *s = &tas_sched[i];
        int mask = 0;
