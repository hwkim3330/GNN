...
이 프로젝트는 복잡하고 동적인 TSN(Time-Sensitive Networking) 환경에서 다수의 데이터 플로우에 대한 최적의 **주 경로(Primary Path) 및 예비 경로(Backup Path)를 자동으로 설정**하는 고성능 지능형 라우팅 솔루션입니다.
...
-   **입력 (Input)**:
    -   `네트워크 토폴로지`: 스위치와 링크의 연결 구조, 링크별 차등 대역폭
    -   `플로우 요구사항`: 수십 ~ 수백 개 데이터 플로우의 출발지, 목적지, 데이터 속성, 데드라인
    -   `고장 시나리오`: 일부 링크 또는 스위치의 장애 상황

-   **핵심 처리 과정 (Processing)**:
    1.  **후보 경로 생성**: GNN이 경로를 직접 만드는 대신, 빠르고 검증된 **K-최단 경로 알고리즘**을 사용해 각 플로우마다 현실적인 후보 경로들을 신속하게 생성합니다.
    2.  **경로 채점 (Path Scoring)**: **PathScorerGNN** 모델이 네트워크 전체의 혼잡도와 후보 경로의 구조를 동시에 분석하여, "이 경로가 현재 상황에서 얼마나 좋은지"에 대한 **정량적인 점수를 예측**합니다.
    3.  **지능형 의사결정**: GNN은 예측된 점수를 바탕으로 모든 후보 경로 중 가장 우수한 경로를 최종 선택합니다.

-   **출력 (Output)**:
    -   `전체 라우팅 솔루션`: 모든 플로우에 할당된 구체적인 주 경로 및 예비 경로 목록.
    -   `성능 평가 보고서`: 생성된 솔루션의 우수성을 나타내는 종합 점수와 계산 시간.
...
1.  **1단계: 전문가 모방 학습 (ILP-Guided Imitation Learning)**
    -   수학적으로 최적해를 보장하는 **ILP(Integer Linear Programming) 솔버**를 '전문가(Oracle)'로 활용합니다.
    -   ILP가 다양한 시나리오에서 생성한 고품질의 경로 결정 데이터를 수집합니다.
    -   GNN은 이 전문가 데이터를 학습하여, ILP의 깊은 "통찰력"과 의사결정 패턴을 모방합니다. 이 단계에서 `CrossEntropyLoss`를 사용하여 전문가가 선택한 경로를 GNN도 선택하도록 훈련합니다.
...
-   **안정적인 아키텍처**: "후보 생성 + 채점" 방식으로 학습의 복잡도를 낮추고, Loss가 안정적으로 수렴하도록 설계되었습니다.
-   **현실적인 시뮬레이션**: 데이터센터의 **팻-트리(Fat-Tree)** 토폴로지, 계층별 차등 대역폭, **핫스팟** 트래픽, **스위치 고장** 등 실제 네트워크의 복잡성을 충실히 반영합니다.
-   **고품질 전문가**: 대역폭 제약조건이 포함된 **ILP 솔버**를 전문가로 사용하여 학습 데이터의 품질을 보장합니다.
-   **완전한 평가 파이프라인**: 최종 학습된 GNN 모델을 전통적인 **Greedy** 방식 및 수학적 최적화 방식인 **ILP**와 성능, 계산 시간, 안정성 측면에서 **3자 비교 벤치마크**를 수행합니다.
...
2.  **실행 과정 및 결과물**:
    ...
    -   **모방 학습**: 생성된 데이터셋으로 GNN을 `IMITATION_EPOCHS` 만큼 학습하고, `MODEL_PATH`에 모델을 저장합니다.
    -   **온라인 학습**: 저장된 모델을 불러와 `ONLINE_EPISODES` 만큼 강화학습을 진행하고, 최종 모델을 `MODEL_PATH`에 덮어씁니다.
