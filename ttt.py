...
# 0. 설정
# ==============================================================================
NUM_DATA_SAMPLES = 50000; ILP_TIME_LIMIT_SEC = 45; NUM_PRODUCERS = max(1, mp.cpu_count() - 1)
IMITATION_EPOCHS = 50; IMITATION_BATCH_SIZE = 128; IMITATION_LR = 1e-4
IMITATION_MODEL_PATH = "gnn_imitator_for_ppo.pth"
RL_EPISODES = 10000; RL_LR = 3e-5; GAMMA = 0.99; EPS_CLIP = 0.2; PPO_UPDATES_PER_EPISODE = 10
...
class ActorCriticGNN(nn.Module):
    def __init__(self, input_dim, hidden_dim=64):
        ...
        self.conv1_actor = GCNConv(input_dim, hidden_dim)
        self.conv2_actor = GCNConv(hidden_dim, hidden_dim)
        self.actor_head = nn.Linear(hidden_dim, 1)
        self.conv1_critic = GCNConv(input_dim, hidden_dim)
        self.conv2_critic = GCNConv(hidden_dim, hidden_dim)
        self.critic_head = nn.Linear(hidden_dim, 1)
    def forward(self, data):
        x_actor = F.relu(self.conv1_actor(data.x, data.edge_index))
        x_actor = F.relu(self.conv2_actor(x_actor, data.edge_index))
        action_logits = self.actor_head(x_actor).squeeze(-1)
        x_critic = F.relu(self.conv1_critic(data.x, data.edge_index))
        x_critic = F.relu(self.conv2_critic(x_critic, data.edge_index))
        state_value = self.critic_head(x_critic)
        return action_logits, state_value
    def evaluate(self, state_batch, action):
        action_logits, state_values = self.forward(state_batch)
        
        # We need to correctly map the actions to their corresponding graphs in the batch
        action_probs = torch.zeros_like(action, dtype=torch.float)
        dist_entropy = torch.zeros(state_batch.num_graphs, dtype=torch.float)
        
        # Process each graph in the batch individually
        node_slices = torch.cat([state_batch.ptr, torch.tensor([state_batch.num_nodes])])
        for i in range(state_batch.num_graphs):
            ...
            # Find the local index of the action
            action_in_graph_local = action[i] - start
            action_probs[i] = dist.log_prob(action_in_graph_local)
            dist_entropy[i] = dist.entropy().mean()

        return action_probs, torch.squeeze(state_values), dist_entropy
...
class Memory:
    ...
    def clear_memory(self): del self.actions[:]; del self.states[:]; del self.logprobs[:]; del self.rewards[:]; del self.is_terminals[:]
        ...
        for flow_id in sorted(profile['flow_definitions'].keys()):
            ...
            if path and len(path) > 1:
                for j in range(len(path) - 1):
                    ...
                    q.put({ 'x': state_tensor.x.tolist(), 'edge_index': state_tensor.edge_index.tolist(), 'y': expert_action })
                partial_paths[flow_id] = {'primary': path}
        break

def generate_dataset_parallel(num_samples):
    print(f"\n===== 병렬 데이터 생성 시작 ({NUM_PRODUCERS}개의 CPU 프로세스) =====")
    q = mp.Queue(); pool = mp.Pool(NUM_PRODUCERS)
    ...
    while len(dataset) < num_samples:
        if not q.empty():
            data_dict = q.get()
            data = Data(x=torch.tensor(data_dict['x'], dtype=torch.float),
...
def train_imitator(model, dataset, epochs):
    ...
    # Custom collate_fn to handle list of Data objects
    def collate(data_list):
        return Batch.from_data_list(data_list)
        
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=IMITATION_BATCH_SIZE, shuffle=True, collate_fn=collate)
    ...
    print(f"\n학습 완료. 최종 모델 저장: '{IMITATION_MODEL_PATH}'")

def run_ppo_training(agent, profile_generator, episodes):
    print("\n===== 2단계: PPO 강화학습 시작 =====")
    agent.policy_old.load_state_dict(torch.load(IMITATION_MODEL_PATH, map_location=DEVICE))
    agent.policy.load_state_dict(torch.load(IMITATION_MODEL_PATH, map_location=DEVICE))
    
    memory = Memory()
    pbar = tqdm(range(episodes), desc="PPO 학습", ncols=100)
    for ep in pbar:
        ...
        for flow_id, props in sorted(profile['flow_definitions'].items()):
            ...
            while not terminal:
                ...
                if action is None or len(current_path) > env.graph.number_of_nodes():
                    memory.rewards.append(-1); memory.is_terminals.append(True); terminal = True # 실패 시 큰 페널티
                else:
                    current_path.append(action)
                    if action == props['dst']:
                        memory.rewards.append(0); memory.is_terminals.append(True); terminal = True
                    else:
                        memory.rewards.append(-0.01); memory.is_terminals.append(False) # 스텝별 작은 페널티
        ...
        memory.rewards[-1] += final_score
        
        if (ep + 1) % 4 == 0:
            agent.update(memory)
            memory.clear_memory()
        
        pbar.set_postfix(score=f"{final_score:.4f}")
...
def run_full_procedure_and_benchmark():
    if not os.path.exists("ilp_dataset.pt"):
        dataset = generate_dataset_parallel(NUM_DATA_SAMPLES)
        torch.save(dataset, "ilp_dataset.pt")
    else:
        print("기존 데이터셋 'ilp_dataset.pt'를 불러옵니다.")
        dataset = torch.load("ilp_dataset.pt")

    ppo_agent = PPOAgent(3, 64, RL_LR, GAMMA, EPS_CLIP, PPO_UPDATES_PER_EPISODE)
